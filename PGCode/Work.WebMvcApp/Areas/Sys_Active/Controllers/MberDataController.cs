...
{
    public class MberDataController : BaseAction<m_Member, a_Member, q_Member, Member>
    {
        ...
        public override ActionResult ListGrid(q_Member sh)
        {
        ...
        public override ActionResult EditMasterNewData()
        {
            ...
            ac = new a_Member() { Connection = getSQLConnection(), logPlamInfo = plamInfo };
            md = new m_Member() { id = ac.GetIDX(ProcCore.Business.CodeTable.Product) };
        ...
        public override ActionResult EditMasterDataByID(int id)
        {
            ...
            RunOneDataEnd<m_Member> HResult = ac.GetDataMaster(id, LoginUserId);
        ...
        protected override void HandleCollectDataToOptions()
        {
            ...
            ViewBag.options_state = MakeCodeToOptions(CodeSheet.MemberState.MakeCodes(), false);
        }
        #endregion
        ...
        [HttpPost]
        [ValidateInput(false)]
        public String ajax_MasterUpdata(m_Member md)
        {
        ...
        /// <returns></returns>
        [HttpGet]
        public override String ajax_MasterGridData(q_Member queryObj)
        {
            #region 連接BusinessLogicLibary資料庫並取得資料
            ac = new a_Member() { Connection = getSQLConnection(), logPlamInfo = plamInfo }; ;

            RunQueryPackage<m_Member> HResult = ac.SearchMaster(queryObj, LoginUserId);
            HandleResultCheck(HResult);
            #endregion
            ...
            foreach (m_Member md in Modules)
            {
                List<String> setFields = new List<String>();

                setFields.Add(md.id.ToString());
                setFields.Add(md.member_name);
                setFields.Add(md.email);
                setFields.Add(GetRecMessage(CodeSheet.MemberState.MakeCodes() ,md.state));
