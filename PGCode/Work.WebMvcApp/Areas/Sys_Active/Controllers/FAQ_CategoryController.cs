...
{
    public class FAQ_CategoryController : BaseAction<m_faq_Category, a_faq_Category, q_faq_Category, FAQ_Category>
    {
        ...
        public override ActionResult ListGrid(q_faq_Category sh)
        {
        ...
        public override ActionResult EditMasterNewData()
        {
            ...
            ac = new a_faq_Category() { Connection = getSQLConnection(), logPlamInfo = plamInfo };
            md = new m_faq_Category() { faq_category_id = ac.GetIDX() };
            md.EditType = ProcCore.Business.Base.EditModeType.Insert;
            #region 新增欄位預設值設定
        ...
        public override ActionResult EditMasterDataByID(int id)
        {
            ...
            RunOneDataEnd<m_faq_Category> HResult = ac.GetDataMaster(id, LoginUserId);
            md = HResult.SearchData;
            md.EditType = ProcCore.Business.Base.EditModeType.Modify;
        ...
        [HttpPost]
        [ValidateInput(false)]
        public String ajax_MasterUpdata(m_faq_Category md)
        {
            ReturnAjaxFiles rAjaxResult = new ReturnAjaxFiles();
            String returnPicturePath = String.Empty;

            a_faq_Category acd = new a_faq_Category() { Connection = getSQLConnection(), logPlamInfo = plamInfo };
            ...
            else
            {
                ...
                rAjaxResult.id = md.faq_category_id;
            }
        ...
        [HttpGet]
        public override String ajax_MasterGridData(q_faq_Category queryObj)
        {
            #region 連接BusinessLogicLibary資料庫並取得資料
            ac = new a_faq_Category() { Connection = getSQLConnection(), logPlamInfo = plamInfo }; ;

            RunQueryPackage<m_faq_Category> HResult = ac.SearchMaster(queryObj, LoginUserId);
            HandleResultCheck(HResult);
            #endregion
            ...
            foreach (m_faq_Category md in Modules)
            {
                List<String> setFields = new List<String>(5);

                setFields.Add(md.faq_category_id.ToString());
                setFields.Add(md.category_name);
                setFields.Add(md.sort.ToString());
                setFields.Add(md.is_open.ToString());
                setRowElement.Add(new RowElement() { id = md.faq_category_id.ToString(), cell = setFields.ToArray() });
            }
            #endregion
            #region 回傳JSON資料
